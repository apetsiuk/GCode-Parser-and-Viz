# Open Source in-Situ Layer-wise 3D Printing Image Anomaly Detection Using HOG and Blender
# https://osf.io/fc42k
...
parser = Parser()
parser.parseFile('C:/.../cornerbracket_gcode_008_0.4n_0.3mm_PLA_MK3_5h43m.gcode')
...
def set_gcode_material():
    ...
    glossy_bsdf.inputs[1].default_value = random.randint(2, 9)/10 #0.8


    diffuse_bsdf.inputs['Color'].default_value = (1,1,1,1)
    diffuse_bsdf.inputs[1].default_value = random.randint(2, 9)/10 #0.7
    mixer_node_1.inputs[0].default_value = random.randint(3, 8)/10 #0.5
    mixer_node_2.inputs[0].default_value = random.randint(3, 8)/10 #0.5
    mixer_node_3.inputs[0].default_value =random.randint(4, 9)/10 #0.7
...
# from https://osf.io/fc42k

def process_layers():
    gcode_mat = set_gcode_material()
...
def animate_layers(start_keyframe):
    ...
    if(len(bpy.data.collections['Layers'].objects)>0):
        for i in range(len(bpy.data.collections['Layers'].objects)):
            ...
            if(ind % 2 == 0):
                ...
                # Create a frame with the cube
                sacrificial_cube = bpy.data.collections['Collection'].objects['Cube']
                sacrificial_cube.select_set(True)
                bpy.context.view_layer.objects.active = sacrificial_cube
                so = bpy.context.active_object
                so.keyframe_insert(data_path="location",frame=start_keyframe+i+1)
                bpy.data.scenes["Scene"].frame_current = start_keyframe+frame_count+1
                sacrificial_cube.select_set(False)
            
                '''
                mask_image = bpy.data.images['Viewer Node']
                mask_image.filepath = f"C:/.../____renders/mask_L{i}.png"
                ...
                so.location[0] = random.randint(1, 240)+0.1
                so.location[1] = random.randint(10, 240)+0.1
                so.location[2] = i/(random.randint(10, 30)/10)+10+random.randint(5, 320)+0.1
    
                angle_x_rad = math.atan((so.location[1]-origin_y)/(so.location[2]-origin_z))
                angle_y_rad = math.atan((so.location[0]-origin_x)/(so.location[2]-origin_z))
                so.rotation_euler[0] = (so.rotation_euler[0]-angle_x_rad)
                so.rotation_euler[1] = (so.rotation_euler[1]+angle_y_rad)
                s_cam.select_set(False)
                #print(math.degrees(angle_x_rad))
                print('Camera moved to ({},{},{}) m(?).'.format(so.location[0],so.location[1],so.location[2]))
            
                bpy.context.scene.render.filepath = f"C:/.../____renders/image_L{i}.png"
...
'''
for m in range(1,len(parser.layers)-0):
